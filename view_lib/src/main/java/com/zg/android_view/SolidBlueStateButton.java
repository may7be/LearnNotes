...
public class SolidBlueStateButton extends StateButton {
    private int mNormalStateColor;
    private int mPressedStateColor;
    private int mUnableStateColor;
    private int mTextColor;

    public SolidBlueStateButton(Context context) {
        this(context, null);
    }

    public SolidBlueStateButton(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SolidBlueStateButton(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        getColor();
        setRadius((int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 0, Resources.getSystem().getDisplayMetrics()));
